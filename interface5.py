...
selected_interface = tk.StringVar()  #binfing the thinkter to python
...
# Available filter options for display
filter_options = ["All", "HTTP", "DNS", "TCP", "UDP", "ICMP", "ARP", "IP Address", "Destination Port"]
selected_filter.set("All")
filter_value = tk.StringVar()  #binding the tkinter to python 

# Clean the packet summary for display
def clean_summary(pkt):
    summary = pkt.summary()
    return summary.replace(" / Raw", "").replace(" / Padding", "")

# Identify the protocol/type of the packet
...
# Get source and destination ports if TCP/UDP is used
def get_ports(pkt):
    if TCP in pkt or UDP in pkt:
        return f"{pkt.sport} → {pkt.dport}"
    return ""

# Check if a packet matches the selected filter
...
# Display the packet list in the GUI
...
# Process each sniffed packet and update the GUI
...
# Apply the selected filter to the captured packets
def apply_filter():
...
# Perform traceroute to the destination IP of the selected packet
...
# Show full detailed info of the selected packet
...
# Show or hide filter value entry based on filter type
...
# Configure background colors based on packet type
...
packet_list.tag_configure("ARP", background="#f5deb3")
packet_list.tag_configure("Other", background="#ffffff")

# Gracefully handle closing the window
def on_close():
